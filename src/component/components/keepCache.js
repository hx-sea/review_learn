...
export const handerReactComponent = (children, prop) =>
	React.isValidElement(children)
		...
		? children(prop)
		: null;
const UpdateComponent = memo(
	({ children }) => children,
	() => true,
);

const CacheKeepItem = memo(
	({ cacheId, children, state, dispatch, lastState, load = () => {}, router = {} }) => {
		const parentCurDom = useRef(null);
		const curDom = useRef(null);
		const curComponent = useRef(null);
		useEffect(() => {
			beforeSwitchDestory[cacheId] = function () {
				if (parentCurDom.current && curDom.current)
		...
		return (
			<div ref={curDom} style={{ display: state === ACTION_UNACTIVED ? 'none' : 'block' }}>
				{state === ACTION_ACTIVE ||
				state === ACTION_ACTIVED ||
				state === ACITON_UNACTIVE ||
				state === ACTION_UNACTIVED ? (
					<UpdateComponent>{children()}</UpdateComponent>
				) : null}
			</div>
		);
	},
...
function Cache({ children, ...prop }) {
	...
	return (
		<CacheContext.Provider value={{ cacheState, cacheDispatch }}>
			...
			{!cacheDispatchCurrent && (
				<GetCacheContext cacheDispatch={(c) => (cacheDispatchCurrent = c)} />
			)}
