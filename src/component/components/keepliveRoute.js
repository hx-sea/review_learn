...
class CacheRoute extends Route {
	...
	constructor(prop, ...arg) {
		...
		if (iskeep && cacheDispatch && cacheState && match) {
			...
			if (
				!cacheState[cacheId] ||
				(cacheState[cacheId] && cacheState[cacheId].state === 'destory')
			) {
	...
	computerMatchRouter = (props) => {
		...
		return computedMatch ? computedMatch : path ? matchPath(location.pathname, props) : null;
	};
	...
	UNSAFE_componentWillReceiveProps(curProps) {
		...
		if (
			this.keepliveState === 'actived' &&
			newMatch &&
			this.cacheMatch &&
			newMatch.path !== this.cacheMatch.path &&
			newMatch.url !== this.cacheMatch.url
		) {
			//TODO: 路由不一致情况
		}
	}
	...
	getAndcheckCacheKey = () => {
		...
		invariant(cacheKey, 'KeepliveRoute must have a cacheId');
		return cacheKey;
	};
...
const KeepliveRoute = (props) => {
	...
	return (
		...
		<Route
			...
			render={(historyProps) => {
				return (
					<CacheRoute {...historyProps} {...props} {...value} iskeep={cacheState ? true : false} />
				);
			}}
		/>
	);
